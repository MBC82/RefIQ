// api/stripe-webhook.js — Stripe webhook handler for Vercel
// Env vars required: STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, FIREBASE_SERVICE_ACCOUNT
//
// Register webhook URL in Stripe Dashboard → Developers → Webhooks:
//   https://www.myrefiq.com/api/stripe-webhook
// Events: checkout.session.completed, customer.subscription.deleted

import { createHmac, timingSafeEqual, createSign } from 'crypto';

// Disable body parser — Stripe signature verification requires the raw body
export const config = {
...
function verifyStripeSignature(rawBody, signatureHeader, secret) {
  ...
  if (ageSecs > 300) throw new Error(`Timestamp too old: ${ageSecs}s`);
...
export default async function handler(req, res) {
  ...
  // Read raw body from stream (bodyParser is disabled above)
  const chunks = [];
  ...
  catch { return res.status(400).end('Invalid JSON body'); }
  ...
  if (stripeEvent.type === 'checkout.session.completed') {
    ...
    if (!userId || !plan) {
      console.warn(`Session ${session.id} missing userId/plan metadata — skipping`);
    ...
    try {
      ...
      if (seasonEnd) { fields.seasonStart = now; fields.seasonEnd = seasonEnd; }

      await firestorePatch('users', userId, fields, token);
      console.log(`✓ users/${userId} → plan=${plan}`);
    } catch (err) {
      console.error('Firestore update failed:', err.message);
      return res.status(500).end('Database update failed');
    }
  }
  ...
  if (stripeEvent.type === 'customer.subscription.deleted') {
    ...
    if (userId) {
      try {
        ...
        console.log(`✓ users/${userId} → plan=expired`);
      } catch (err) {
